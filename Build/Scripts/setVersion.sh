...
# ---------------------------------------------------------------------------
#    Fixture extensions live in a Fixtures/Extensions/ folder next to the
#    functional tests using them, at any depth below Tests/Functional/.
FIXTURE_BASE_DIR="Tests/Functional"
declare -a FIXTURE_DIRS=()
if [ -d "${FIXTURE_BASE_DIR}" ]; then
    while IFS= read -r fixtureDir; do
        FIXTURE_DIRS+=("${fixtureDir}")
    done < <(find "${FIXTURE_BASE_DIR}" -type d -path '*/Fixtures/Extensions/*' ! -path '*/Fixtures/Extensions/*/*' | sort)
fi
