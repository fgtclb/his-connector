...
readonly class PersonFactory
{
    ...
    public function create(PersonExisting $person, string $language): Person
    {
        return new Person(
            ...
            fetchContactDetailsClosure: fn() => $this->contactDetailsRepository->findByPersonId($person->getId(), $language),
            fetchPicturesClosure: fn(int $hisKey) => $this->personPictureRepository->findPicturesForPerson($person->getId(), $hisKey),
            fetchFunctionsClosure: fn() => $this->personFunctionRepository->findFunctionsForPerson($person->getId(), $language),
        );
    }
}
