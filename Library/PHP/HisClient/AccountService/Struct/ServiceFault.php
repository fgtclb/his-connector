...
class ServiceFault extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \FGTCLB\HisClient\AccountService\Struct\ValidationMessage[]
     */
     ...
     * @param \FGTCLB\HisClient\AccountService\Struct\ValidationMessage[] $validationMessage
     */
     ...
     * Set message value
     * @param string $message
     * @return \FGTCLB\HisClient\AccountService\Struct\ServiceFault
     */
     ...
     * this property may have been unset before, due to the fact that this property is
     * removable from the request (nillable=true+minOccurs=0)
     * @return \FGTCLB\HisClient\AccountService\Struct\ValidationMessage[]
     */
    ...
    public static function validateValidationMessageForArrayConstraintFromSetValidationMessage(?array $values = []): string
    {
        ...
        foreach ($values as $serviceFaultValidationMessageItem) {
            ...
            if (!$serviceFaultValidationMessageItem instanceof \FGTCLB\HisClient\AccountService\Struct\ValidationMessage) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The validationMessage property can only contain items of type \FGTCLB\HisClient\AccountService\Struct\ValidationMessage, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \FGTCLB\HisClient\AccountService\Struct\ValidationMessage $item
     * @return \FGTCLB\HisClient\AccountService\Struct\ServiceFault
     */
    public function addToValidationMessage(\FGTCLB\HisClient\AccountService\Struct\ValidationMessage $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \FGTCLB\HisClient\AccountService\Struct\ValidationMessage) {
            throw new InvalidArgumentException(sprintf('The validationMessage property can only contain items of type \FGTCLB\HisClient\AccountService\Struct\ValidationMessage, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
