...
class ServiceFault extends AbstractStructBase
{
     ...
     * - maxOccurs: unbounded
     * - minOccurs: 0
     * @var \FGTCLB\HisClient\FacilityService\Struct\ValidationMessage[]
     */
     ...
     * @param \FGTCLB\HisClient\FacilityService\Struct\ValidationMessage[] $validationMessage
     */
     ...
     * Set message value
     * @param string $message
     * @return \FGTCLB\HisClient\FacilityService\Struct\ServiceFault
     */
    ...
    public static function validateValidationMessageForArrayConstraintFromSetValidationMessage(?array $values = []): string
    {
        ...
        foreach ($values as $serviceFaultValidationMessageItem) {
            ...
            if (!$serviceFaultValidationMessageItem instanceof \FGTCLB\HisClient\FacilityService\Struct\ValidationMessage) {
        ...
        if (!empty($invalidValues)) {
            $message = sprintf('The validationMessage property can only contain items of type \FGTCLB\HisClient\FacilityService\Struct\ValidationMessage, %s given', is_object($invalidValues) ? get_class($invalidValues) : (is_array($invalidValues) ? implode(', ', $invalidValues) : gettype($invalidValues)));
        }
        unset($invalidValues);
     ...
     * @throws InvalidArgumentException
     * @param \FGTCLB\HisClient\FacilityService\Struct\ValidationMessage $item
     * @return \FGTCLB\HisClient\FacilityService\Struct\ServiceFault
     */
    public function addToValidationMessage(\FGTCLB\HisClient\FacilityService\Struct\ValidationMessage $item): self
    {
        // validation for constraint: itemType
        if (!$item instanceof \FGTCLB\HisClient\FacilityService\Struct\ValidationMessage) {
            throw new InvalidArgumentException(sprintf('The validationMessage property can only contain items of type \FGTCLB\HisClient\FacilityService\Struct\ValidationMessage, %s given', is_object($item) ? get_class($item) : (is_array($item) ? implode(', ', $item) : gettype($item))), __LINE__);
        }
