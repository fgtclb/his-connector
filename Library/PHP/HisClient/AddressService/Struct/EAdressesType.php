...
/**
 * This class stands for eAdressesType Struct
 * @subpackage Structs
 */
#[\AllowDynamicProperties]
class EAdressesType extends AbstractStructBase
{
    /**
     * The eAddress
    ...
    /**
     * Constructor method for eAdressesType
     * @uses EAdressesType::setEAddress()
    ...
    public static function validateEAddressForArrayConstraintFromSetEAddress(?array $values = []): string
    {
        ...
        $message = '';
        $invalidValues = [];
        foreach ($values as $eAdressesTypeEAddressItem) {
            // validation for constraint: itemType
            if (!$eAdressesTypeEAddressItem instanceof \FGTCLB\HisClient\AddressService\Struct\EAddress) {
                $invalidValues[] = is_object($eAdressesTypeEAddressItem) ? get_class($eAdressesTypeEAddressItem) : sprintf('%s(%s)', gettype($eAdressesTypeEAddressItem), var_export($eAdressesTypeEAddressItem, true));
            }
        }
